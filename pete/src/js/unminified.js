...
var FWDButtonToolTip = function(
		...
		self.setupMainContainers = function(){	
			...
			self.middle_sdo.setBkColor("#FFFFFF");
		...
		self.show = function(){
			...
			FWDAnimation.to(self, .4, {alpha:1, delay:.1, ease:Quart.easeOut});
...
var FWDController = function(
		...
		this.controllerVerticalPosition_str  = data.controllerVerticalPosition_str; 
		...
		this.controllerOffsetX = data.controllerOffsetX;
		...
		this.minWidth;
		this.controllerHorizontalPosition_str = data.controllerHorizontalPosition_str;
		...
		for(var i=0; i<data.buttons_ar.length; i++){
			...
			if(data.buttons_ar[i] == "rotate"){
				this.buttonWidth = data.controllerRotateN_img.width;
				this.buttonHeight = data.controllerRotateN_img.height;
			}
			
			if(data.buttons_ar[i] == "pan"){
				this.buttonWidth = data.controllerPanN_img.width;
				this.buttonHeight = data.controllerPanN_img.height;
			}
			
			if(data.buttons_ar[i] == "roteteleft"){
				this.buttonWidth = data.controllerPrevN_img.width;
				this.buttonHeight = data.controllerPrevN_img.height;
			}
			
			if(data.buttons_ar[i] == "rotateright"){
				this.buttonWidth = data.controllerNextN_img.width;
				this.buttonHeight = data.controllerNextN_img.height;
			}
					
			if(data.buttons_ar[i] == "play"){
				this.buttonWidth = data.controllerPlayN_img.width;
				this.buttonHeight = data.controllerPlayN_img.height;
			}
			...
			if(data.buttons_ar[i] == "info"){
				this.buttonWidth = data.controllerInfoN_img.width;
				this.buttonHeight = data.controllerInfoN_img.height;
			}
			
			if(data.buttons_ar[i] == "link"){
				this.buttonWidth = data.controllerLinkN_img.width;
				this.buttonHeight = data.controllerLinkN_img.height;
			}
			
			if(data.buttons_ar[i] == "fullscreen"){
				this.buttonWidth = data.controllerFullScreenNormalN_img.width;
				this.buttonHeight = data.controllerFullScreenNormalN_img.height;
			}
		}
		...
		this.onKeyDownHandler = function(e){
		    ...
		    if(e.keyCode == 38){
				...
				self.gotoImageId_to = setTimeout(self.goToTopImageWithDelay, 400);
				...
				self.isKeyPressed_bl = true;
				if(self.slideShowButton_do) self.stopSlideShow();
				self.goToBottomImage();
				clearInterval(self.gotoImageId_int);
				clearTimeout(self.gotoImageId_to);
				self.gotoImageId_to = setTimeout(self.goToBottomImageWithDelay, 400);
				self.dispatchEvent(FWDController.GOTO_NEXT_OR_PREV_IMAGE_COMPLETE);
		...
		self.positionButtons = function(){
			...
			if(self.controllerHorizontalPosition_str == "center"){
				self.mainHolder_do.setX(Math.round((self.stageWidth - self.curWidth)/2));
			}else if(self.controllerHorizontalPosition_str == "left"){
				self.mainHolder_do.setX(self.controllerOffsetX);
			}else if(self.controllerHorizontalPosition_str == "right"){
				self.mainHolder_do.setX(Math.round((self.stageWidth - self.curWidth)) - self.controllerOffsetX);
			}
		
			
			if(self.controllerVerticalPosition_str == FWDController.POSITION_TOP){
		...
		self.setupPanButton = function(toolTipLabel){
			...
			if(self.showButtonsLabels_bl){
				...
				self.panButtonTooTipLabel_do = new FWDButtonToolTip(
						...
						self.controllerVerticalPosition_str, 
		...
		self.goToTopImage = function(){
			var dir = 1;
			if(self.inverseNextAndPrevRotation_bl) dir = -1; 
			self.dispatchEvent(FWDController.GOTO_TOP_OR_BOTTOM_IMAGE, {dir:dir});
		};
		
		self.goToTopImageWithDelay = function(){
			self.gotoImageId_int = setInterval(self.goToTopImage, self.rotationSpeed);
		};
		
		self.goToBottomImage = function(){
			var dir = -1;
			if(self.inverseNextAndPrevRotation_bl) dir = 1; 
			self.dispatchEvent(FWDController.GOTO_TOP_OR_BOTTOM_IMAGE, {dir:dir});
		};
		
		self.goToBottomImageWithDelay = function(){
			self.gotoImageId_int = setInterval(self.goToBottomImage, self.rotationSpeed);
		};
		...
		self.stopSlideShow = function(){
			self.enablePlayToImage_bl = false;
			self.dir = 1;
		...
		self.slideShowComplete = function(){
			self.dispatchEvent(FWDController.GOTO_NEXT_OR_PREV_IMAGE, {dir:self.dir});
			self.dispatchEvent(FWDController.GOTO_NEXT_OR_PREV_IMAGE_COMPLETE);
		
			
			if(self.enablePlayToImage_bl){
				if(parent.imageManager_do.curId == self.idToStop){
					self.stopSlideShow();
					eval(self.callBackFunction_str + "()");
				}
			}
		};
		
		this.idToStop;
		this.enablePlayToImage_bl = false;
		this.dir = 1;
		this.callBackFunction_str;
		
		this.playToImage = function(id, callBack){
			if(id == parent.imageManager_do.curId) return;
			self.enablePlayToImage_bl = true;
			self.idToStop = id;
			self.callBackFunction_str = callBack;
			
			if(id == 0){
				id = 0;
			}else if(id > self.totalImages - 1){
			...
			if(self.stopRotationAtEnds_bl){
				...
				var d1 = Math.abs(parent.imageManager_do.curId - self.idToStop);
				var d2 = Math.abs(parent.imageManager_do.curId - (self.idToStop +  self.totalImages));
				
				if(d1 > d2){
					if(self.idToStop < parent.imageManager_do.curId){
						self.dir = 1;
					}else{
						self.dir = -1;
					}
		...
		self.setupLinkButton = function(toolTipLabel){
			...
			self.linkButton_do.addListener(FWDComplexButton.MOUSE_DOWN, self.linkButtonOnMouseClickHandler);
		...
		self.handlerOnMouseOut = function(e){
			if(self.showButtonsLabels_bl && !self.isHandlerClicked_bl) self.scrollBarHandlerToolTip_do.hide();
		};
		...
		self.handlerDragEndHandler = function(e){
			var viewportMouseCoordinates;
			self.isHandlerClicked_bl = false;
			self.dispatchEvent(FWDController.ENABLE_PAN_OR_MOVE);
			viewportMouseCoordinates = FWDUtils.getViewportMouseCoordinates(e);	
		...
		self.updateScrollBar = function(percent, animate){
			...
			FWDAnimation.killTweensOf(self.scrollBarHandler_do);
			if(animate){
				FWDAnimation.to(self.scrollBarHandler_do, .2, {x:self.finalHandlerX, 
		...
		self.positionAndSetLabelScrollBarHandler = function(){
			...
			setTimeout(function(){
				...
				if(self.controllerVerticalPosition_str ==  FWDController.POSITION_BOTTOM){
		...
		self.hide = function(animate){
			if(self.controllerVerticalPosition_str == FWDController.POSITION_BOTTOM){
				if(animate){
					FWDAnimation.to(self.mainHolder_do, 1, {y:self.curHeight + self.controllerOffsetY, ease:Expo.easeInOut});
				}else{
					self.mainHolder_do.setY(self.curHeight + self.controllerOffsetY);
				}
			}else if(self.controllerVerticalPosition_str == FWDController.POSITION_TOP){
				if(animate){
					FWDAnimation.to(self.mainHolder_do, 1, {y:-self.curHeight - self.controllerOffsetY, ease:Expo.easeInOut});
				}else{
					self.mainHolder_do.setY(-self.curHeight - self.controllerOffsetY);
		...
		self.show = function(){
			FWDAnimation.to(self.mainHolder_do, 1, {y:0, ease:Expo.easeInOut});
		};
		...
		this.destroy = function(){
			...
			self.draggingMode_str = null;
			self.controllerVerticalPosition_str = null;
...
(function(window){
	...
	var FWDData = function(props){
		...
		this.controllerVerticalPosition_str = null;
		...
		this.stopRotationHorizontalAtEnds_bl = false;
		this.slideShowAutoPlay_bl = false;
		this.isMobile_bl = FWDUtils.isMobile;
		this.hasPointerEvent_bl = FWDUtils.hasPointerEvent;
		this.areAllImagesLoaded_bl = false;
		this.isLigthboxCloseButtonLoaded_bl = false;
		...
		self.parseProperties = function(){
			...
			self.rootElement_el = FWDUtils.getChildById(self.props_obj.playListId);
			if(!self.rootElement_el){
				errorMessage_str = "Make sure that the a div with the id - <font color='#FF0000'>" + self.props_obj.playListId + "</font> exists, self represents the data playlist.";
				self.dispatchEvent(FWDData.LOAD_ERROR, {text:errorMessage_str});
				return;
			}
			...
			if(!playListElement_el){
				errorMessage_str = "Make sure that the an ul tag with the data type attribute - <font color='#FF0000'>data-paylist</font> is defined, this tag is used for creating the playlist.";
				self.dispatchEvent(FWDData.LOAD_ERROR, {text:errorMessage_str});
				return;
			}
			...
			self.dragVertical_bl = self.props_obj.dragVertical == "yes" ? true : false;
	
			
			self.disableMouseWheelZoom_bl = self.props_obj.disableMouseWheelZoom == "yes" ? true : false;
			...
			self.stopRotationHorizontalAtEnds_bl = self.props_obj.stopRotationHorizontalAtEnds == "yes" ? true : false;
			self.stopRotationVerticalAtEnds_bl = self.props_obj.stopRotationVerticalAtEnds == "yes" ? true : false;
			
			self.addDragAndSpinSupport_bl = self.props_obj.addDragAndSpinSupport == "yes" ? true : false;
			self.zoomSpeed = self.props_obj.zoomSpeed || 0.1;
			
			self.navigatorWidth = self.props_obj.navigatorWidth || 120;
			if(self.stopRotationHorizontalAtEnds_bl) self.addDragAndSpinSupport_bl = false;
			...
			self.totalMainPlaylists = playListChildren_ar.length;
			
			self.playListData_ar = [];
			for(var x=0; x<self.totalMainPlaylists; x++){
			   ...
			   // console.log(playlist);
				...
				for(var i=0; i<totalPlaylistItems; i++){
					playlistChild =  FWDUtils.getChildren(playlist[i]);
						...
						for(var z=0; z<playlistChild.length; z++){
							...
							if(!test){
								self.dispatchEvent(FWDData.LOAD_ERROR, {text:"Element with attribute <font color='#FF0000'> data-small-image-path</font> is not defined in playlist <font color='#FF0000'>" + playlistPositionError +  "</font> at positon <font color='#FF0000'>" + itemPositionError + "</font>"});
								return;
							}
							...
							if(!test){
								self.dispatchEvent(FWDData.LOAD_ERROR, {text:"Element with attribute <font color='#FF0000'> data-large-image-path</font> is not defined in playlist <font color='#FF0000'>" + playlistPositionError +  "</font> at positon <font color='#FF0000'>" + itemPositionError + "</font>"});
								return;
							}
				...
				self.playListData_ar[x] = playlist_ar;
			}
			...
			self.controllerVerticalPosition_str = self.props_obj.controllerVerticalPosition || "bottom";
			if(self.controllerVerticalPosition_str != "top" && self.controllerVerticalPosition_str != "bottom") self.startDraggingMode_str = "top";
			self.controllerHorizontalPosition_str = self.props_obj.controllerHorizontalPosition || "center";
			
		
			if(self.props_obj.buttons === undefined){
				errorMessage_str = "The <font color='#FF0000'>buttons</font> is not defined in the contructor, this is necessary to setup the main buttons.";
				self.dispatchEvent(FWDData.LOAD_ERROR, {text:errorMessage_str});
				console.log(errorMessage_str);
				return;
			} 
			
			if(self.props_obj.buttons.length > 0){
			...
			if(self.controllerBackgroundOpacity == undefined)  self.controllerBackgroundOpacity = 1;
			...
			self.controllerOffsetX = self.props_obj.controllerOffsetX || 0; 
			...
			//setup skin paths
			self.handMovePath_str = self.skinPath_str + "skin/handmove.cur";
			self.handGrabRotatePath_str = self.skinPath_str + "skin/handrotate.cur";
			self.handGrabRotateVerticalPath_str = self.skinPath_str + "skin/handrotatetop.cur";
			
			self.handGrabPath_str = self.skinPath_str +  "skin/handgrab.cur";
			var preloaderPath_str = self.skinPath_str +  "skin/preloader.png";
			var mainLightBoxCloseButtonNPath_str = self.skinPath_str +  "skin/close-icon.png";
			var mainLightBoxCloseButtonSPath_str = self.skinPath_str + "skin/close-icon-rollover.png";
			var controllerBackgroundLeftPath_str = self.skinPath_str + "skin/bg-bar-left.png";
			var controllerBackgroundRight_str = self.skinPath_str + "skin/bg-bar-right.png";
			var controllerPanN_str = self.skinPath_str + "skin/move-icon.png";
			var controllerPanS_str = self.skinPath_str +  "skin/move-icon-rollover.png";
			var controllerRotateN_str = self.skinPath_str + "skin/rotate-icon.png";
			var controllerRotateS_str = self.skinPath_str + "skin/rotate-icon-rollover.png";
			var controllerNextN_str = self.skinPath_str + "skin/right-icon.png";
			var controllerNextS_str = self.skinPath_str +  "skin/right-icon-rollover.png";
			var controllerPrevN_str = self.skinPath_str + "skin/left-icon.png";
			var controllerPrevS_str = self.skinPath_str + "skin/left-icon-rollover.png";
			var controllerPlayN_str = self.skinPath_str +  "skin/play-icon.png";
			var controllerPlayS_str = self.skinPath_str +  "skin/play-icon-rollover.png";
			var controllerPauseN_str = self.skinPath_str + "skin/pause-icon.png";
			var controllerPauseS_str = self.skinPath_str + "skin/pause-icon-rollover.png";
			var controllerInfoN_str = self.skinPath_str + "skin/info-icon.png";
			var controllerInfoS_str = self.skinPath_str + "skin/info-icon-rollover.png";
			var controllerLinkN_str = self.skinPath_str + "skin/link-icon.png";
			var controllerLinkS_str = self.skinPath_str + "skin/link-icon-rollover.png";
			var controllerFullScreemNormalN_str = self.skinPath_str + "skin/fullscr-normal-icon.png";
			var controllerFullScreenNormalS_str = self.skinPath_str +  "skin/fullscr-normal-icon-rollover.png";
			var controllerFullScreemFullN_str = self.skinPath_str + "skin/fullscr-full-icon.png";
			var controllerFullScreenFullS_str = self.skinPath_str + "skin/fullscr-full-icon-rollover.png";
			var zoomInN_str = self.skinPath_str + "skin/zoomin.png";
			var zoomInS_str = self.skinPath_str +  "skin/zoomin-rollover.png";
			var zoomOutN_str = self.skinPath_str + "skin/zoomout.png";
			var zoomOutS_str = self.skinPath_str + "skin/zoomout-rollover.png";
			var scrollBarHandlerN_str = self.skinPath_str + "skin/handler.png";
			var scrollBarHandlerS_str = self.skinPath_str + "skin/handler-rollover.png";
			var scrollBarLeft_str = self.skinPath_str + "skin/scrool-left.png";
			var scrollBarRight_str = self.skinPath_str + "skin/scrool-right.png";
			self.scrollBarMiddlePath_str = self.skinPath_str + "skin/scrool-middle.png";
			self.controllerBackgroundMiddlePath_str = self.skinPath_str + "skin/bg-bar-middle.png";
			self.buttonToolTipLeft_str = self.skinPath_str +  "skin/button-tool-tip-left.png";
			self.buttonToolTipMiddle_str = self.skinPath_str + "skin/button-tool-tip-middle.png";
			self.buttonToolTipRight_str = self.skinPath_str +  "skin/button-tool-tip-right.png";
			self.buttonToolTipBottomPointer_str = self.skinPath_str + "skin/button-tool-tip-down-pointer.png";
			self.buttonToolTipTopPointer_str = self.skinPath_str + "skin/button-tool-tip-top-pointer.png";
			var infoWindowCloseNormal_str = self.skinPath_str + "skin/close-icon.png";
			var infoWindowCloseSelected_str = self.skinPath_str + "skin/close-icon-rollover.png";
			...
			self.mainPreloader_img.src = preloaderPath_str;
			
			self.skinPaths_ar = [
			     {img:self.mainLightboxCloseButtonN_img = new Image(), src:mainLightBoxCloseButtonNPath_str},
			     {img:self.mainLightboxCloseButtonS_img = new Image(), src:mainLightBoxCloseButtonSPath_str},
			     {img:self.controllerBackgroundLeft_img = new Image(), src:controllerBackgroundLeftPath_str},
			     {img:self.controllerBackgroundRight_img = new Image(), src:controllerBackgroundRight_str},
			     {img:self.toolTipPointer_img = new Image(), src:self.buttonToolTipTopPointer_str},
			     {img:self.toolTipLeft_img = new Image(), src:self.buttonToolTipLeft_str},
			];
			
			for(var i=0; i<self.buttons_ar.length; i++){
			
				if(self.buttons_ar[i] == "rotate"){
					self.skinPaths_ar.push({img:self.controllerRotateN_img = new Image(), src:controllerRotateN_str});
					self.skinPaths_ar.push({img:self.controllerRotateS_img = new Image(), src:controllerRotateS_str});
				}
				
				if(self.buttons_ar[i] == "pan"){
					self.skinPaths_ar.push({img:self.controllerPanN_img = new Image(), src:controllerPanN_str});
					self.skinPaths_ar.push({img:self.controllerPanS_img = new Image(), src:controllerPanS_str});
				}
				
				if(self.buttons_ar[i] == "roteteleft"){
					self.skinPaths_ar.push( {img:self.controllerPrevN_img = new Image(), src:controllerPrevN_str});
					self.skinPaths_ar.push( {img:self.controllerPrevS_img = new Image(), src:controllerPrevS_str});
				}
				
				if(self.buttons_ar[i] == "rotateright"){
					self.skinPaths_ar.push( {img:self.controllerNextN_img = new Image(), src:controllerNextN_str});
					self.skinPaths_ar.push({img:self.controllerNextS_img = new Image(), src:controllerNextS_str});
				}
				
				if(self.buttons_ar[i] == "scrollbar"){
					self.skinPaths_ar.push({img:self.scrollBarHandlerN_img = new Image(), src:scrollBarHandlerN_str});
					self.skinPaths_ar.push({img:self.scrollBarHandlerS_img = new Image(), src:scrollBarHandlerS_str});
					self.skinPaths_ar.push({img:self.scrollBarLeft_img = new Image(), src:scrollBarLeft_str});
					self.skinPaths_ar.push({img:self.scrollBarRight_img = new Image(), src:scrollBarRight_str});
					self.skinPaths_ar.push({img:self.zoomInN_img = new Image(), src:zoomInN_str});
					self.skinPaths_ar.push({img:self.zoomInS_img = new Image(), src:zoomInS_str});
					self.skinPaths_ar.push({img:self.zoomOutN_img = new Image(), src:zoomOutN_str});
					self.skinPaths_ar.push({img:self.zoomOutS_img = new Image(), src:zoomOutS_str});
				}
				
				if(self.buttons_ar[i] == "play"){
					self.skinPaths_ar.push({img:self.controllerPlayN_img = new Image(), src:controllerPlayN_str});
					self.skinPaths_ar.push({img:self.controllerPlayS_img = new Image(), src:controllerPlayS_str});
					self.skinPaths_ar.push({img:self.controllerPauseN_img = new Image(), src:controllerPauseN_str});
					self.skinPaths_ar.push({img:self.controllerPauseS_img = new Image(), src:controllerPauseS_str});
				}
				
				if(self.buttons_ar[i] == "info"){
					self.skinPaths_ar.push({img:self.controllerInfoN_img = new Image(), src:controllerInfoN_str});
					self.skinPaths_ar.push({img:self.controllerInfoS_img = new Image(), src:controllerInfoS_str});
					self.skinPaths_ar.push({img:self.infoWindowCloseNormal_img = new Image(), src:infoWindowCloseNormal_str});
					self.skinPaths_ar.push({img:self.infoWindowCloseSelected_img = new Image(), src:infoWindowCloseSelected_str});
				}
				
				if(self.buttons_ar[i] == "link"){
					self.skinPaths_ar.push({img:self.controllerLinkN_img = new Image(), src:controllerLinkN_str});
					self.skinPaths_ar.push({img:self.controllerLinkS_img = new Image(), src:controllerLinkS_str});
				}
				
				if(self.buttons_ar[i] == "fullscreen"){
					self.skinPaths_ar.push({img:self.controllerFullScreenNormalN_img = new Image(), src:controllerFullScreemNormalN_str});
					self.skinPaths_ar.push({img:self.controllerFullScreenNormalS_img = new Image(), src:controllerFullScreenNormalS_str});
					self.skinPaths_ar.push({img:self.controllerFullScreenFullN_img = new Image(), src:controllerFullScreemFullN_str});
					self.skinPaths_ar.push({img:self.controllerFullScreenFullS_img = new Image(), src:controllerFullScreenFullS_str});
				}
			...
			self.totalGraphics = self.skinPaths_ar.length;
				...
				//set images paths.
			for(var i=0; i<self.playListData_ar.length; i++){
				for(var j=0; j<self.playListData_ar[i].length; j++){
					self.imagesPaths_ar.push(self.playListData_ar[i][j].smallImagePath);
				}
			}
			self.totalImages = self.imagesPaths_ar.length;
			
			self.startAtImage = self.props_obj.startAtImage || 0;
			if(self.startAtImage < 0){
				self.staetAtImage = 0;
			}else if(self.startAtImage > self.playListData_ar[0].length - 1){
				self.startAtImage = self.playListData_ar[0].length - 1;
			}
			
			self.startAtSet = self.props_obj.startAtSet || 0;
			if(self.startAtSet < 0){
				self.startAtSet = 0;
			}else if(self.startAtSet > self.playListData_ar.length - 1){
				self.startAtSet = self.playListData_ar.length - 1;
			}
			
			if(self.playListData_ar.length > 1) self.dragInAllDirection_bl = true;
		...
		this.onSkinLoadHandler = function(e){
			if(self.mainLightboxCloseButtonS_img && !self.isLigthboxCloseButtonLoaded_bl){
				self.dispatchEvent(FWDData.LIGHBOX_CLOSE_BUTTON_LOADED);
				self.isLigthboxCloseButtonLoaded_bl = true;
			}
			if(self.controllerBackgroundLeft_img && !self.controllerHeight){
		...
		self.onNavigatorImageLoadErrorHandler = function(e){
			var message = "The navigator image with the label <font color='#FF0000'>" + self.navigatorImagesPaths_ar[self.countLoadedImages] + "</font> can't be loaded, make sure that the image exists and the path is correct!";
		...
		self.onImageLoadErrorHandler = function(e){
			var message = "The image with the label <font color='#FF0000'>" + self.imagesPaths_ar[self.countLoadedImages] + "</font> can't be loaded, make sure that the image exists and the path is correct!";
			console.log(e);
			var err = {text:message};
			self.dispatchEvent(FWDData.LOAD_ERROR, err);
		};
		...
		self.checkForAttribute = function(e, attr, position){
			...
			if(!test){
				if(position != undefined){
					self.dispatchEvent(FWDData.LOAD_ERROR, {text:"Element with attribute <font color='#FF0000'>" + attr + "</font> is not defined at positon <font color='#FF0000'>" + (position + 1) + "</font>"});
				}else{
					self.dispatchEvent(FWDData.LOAD_ERROR, {text:"Element with attribute <font color='#FF0000'>" + attr + "</font> is not defined."});
				}
				return;
			}
			return test;
		...
		self.showPropertyError = function(error){
			self.dispatchEvent(FWDData.LOAD_ERROR, {text:"The property called <font color='#FF0000'>" + error + "</font> is not defined."});
		};
		
		self.showMarkerError = function(error, position){
			self.dispatchEvent(FWDData.LOAD_ERROR, {text:"The marker at position <font color='#FF0000'>" + position + "</font> dose not have defined an attribute <font color='#FF0000'>" + error + "</font>."});
		};
		
		self.showMarkerTypeError = function(error, position){
			self.dispatchEvent(FWDData.LOAD_ERROR, {text:"Marker type is incorrect <font color='#FF0000'>" + error + "</font> at position <font color='#FF0000'>" + position + "</font>. Accepted types are <font color='#FF0000'>link, tooltip, infowindow</font>."});
		};
		...
		self.destroy = function(){
			...
			if(self.mainPreloader_img) self.mainPreloader_img.src = "";
			if(self.mainLightboxCloseButtonN_img) self.mainLightboxCloseButtonN_img.src = "";
			if(self.mainLightboxCloseButtonS_img) self.mainLightboxCloseButtonS_img.src = "";
			if(self.controllerBackgroundLeft_img) self.controllerBackgroundLeft_img.src = ""; 
			if(self.controllerBackgroundRight_img) self.controllerBackgroundRight_img.src = ""; 
			if(self.controllerPanN_img) self.controllerPanN_img.src = ""; 
			if(self.controllerPanS_img) self.controllerPanS_img.src = "";
			
			if(self.controllerRotateN_img) self.controllerRotateN_img.src = "";
			if(self.controllerRotateS_img) self.controllerRotateS_img.src = "";
			if(self.controllerNextN_img) self.controllerNextN_img.src = "";
			if(self.controllerNextS_img) self.controllerNextS_img.src = "";
			if(self.controllerPrevN_img) self.controllerPrevN_img.src = "";
			if(self.controllerPrevS_img) self.controllerPrevS_img.src = "";
			if(self.controllerPlayN_img) self.controllerPlayN_img.src = "";
			if(self.controllerPlayS_img) self.controllerPlayS_img.src = "";
			if(self.controllerPauseN_img) self.controllerPauseN_img.src = "";
			if(self.controllerPauseS_img) self.controllerPauseS_img.src = "";
			if(self.controllerInfoN_img) self.controllerInfoN_img.src = "";
			if(self.controllerLinkN_img) self.controllerLinkN_img.src = "";
			if(self.controllerLinkS_img) self.controllerLinkS_img.src = "";
			if(self.controllerFullScreenNormalN_img) self.controllerFullScreenNormalN_img.src = "";
			if(self.controllerFullScreenNormalS_img) self.controllerFullScreenNormalS_img.src = "";
			if(self.controllerFullScreenFullN_img) self.controllerFullScreenFullN_img.src = "";
			if(self.controllerFullScreenFullS_img) self.controllerFullScreenFullS_img.src = "";
			if(self.zoomInN_img) self.zoomInN_img.src = "";
			if(self.zoomInS_img) self.zoomInS_img.src = "";
			if(self.zoomOutN_img) self.zoomOutN_img.src = "";
			if(self.zoomOutS_img) self.zoomOutS_img.src = "";
			if(self.scrollBarHandlerN_img) self.scrollBarHandlerN_img.src = "";
			if(self.scrollBarHandlerN_img) self.scrollBarHandlerN_img.src = "";
			if(self.scrollBarHandlerS_img) self.scrollBarHandlerS_img.src = "";
			if(self.scrollBarLeft_img) self.scrollBarLeft_img.src = "";
			if(self.scrollBarLeft_img) self.scrollBarLeft_img.src = "";
			if(self.scrollBarRight_img) self.scrollBarRight_img.src = "";
			if(self.toolTipLeft_img) self.toolTipLeft_img.src = "";
			if(self.toolTipPointer_img) self.toolTipPointer_img.src = "";
			if(self.infoWindowCloseNormal_img) self.infoWindowCloseNormal_img.src = "";
			if(self.infoWindowCloseSelected_img) self.infoWindowCloseSelected_img.src = "";
...
var FWDDescriptionWindow = function(parent, data){
		...
		if(!self.infoWindowCloseNormal_img) return;
		...
		self.updateSize = function(){
			...
			if(self.isMobile_bl){	
				setTimeout(function(){
					if(self == null) return;
					FWDAnimation.killTweensOf(self.mainContentHolder_do);
		...
		self.scrollBarHandlerOnMouseOver = function(){
			FWDAnimation.to(self.scrollBarHandler_sdo, .2, {alpha:1, w:10});
			FWDAnimation.to(self.scrollBarTrack_sdo, .2, {alpha:.4, w:10});
			FWDAnimation.to(self.scrollBar_do, .2, {x:self.mainContentHolderWidth - self.scrollBarWidth - 6});
		};
		
		self.scrollBarHandlerOnMouseOut = function(){
			if(self.isDragging_bl) return;
			FWDAnimation.to(self.scrollBarHandler_sdo, .3, {alpha:.5, w:self.scrollBarWidth});
			FWDAnimation.to(self.scrollBarTrack_sdo, .3, {alpha:0, w:self.scrollBarWidth});
			FWDAnimation.to(self.scrollBar_do, .3, {x:self.mainContentHolderWidth - self.scrollBarWidth - 2});
		};
		...
		self.updatePCHandler = function(animate){
			...
			if(animate){
				FWDAnimation.to(self.contentHolder_sdo, .3, {y:Math.round(self.contentFinalY)});
			}else{
		...
		self.show = function(text){
			...
			if(FWDUtils.isMobile){
				FWDAnimation.to(self.background_sdo, .8, {alpha:self.backgroundOpacity, delay:.2});
				self.showOrHideWithDelayId_to = setTimeout(self.showWithDelay, 1800);
			}else{
				FWDAnimation.to(self.background_sdo, .6, {alpha:self.backgroundOpacity});
		...
		self.showWithDelay = function(){
			...
			FWDAnimation.to(self.mainContentHolder_do, .8, {y:self.mainContentFinalY, ease:Expo.easeInOut});
		};
		
		self.hide = function(animate, overwrite){
			if(!self.isShowed_bl && !overwrite) return;
			FWDAnimation.killTweensOf(self.background_sdo);	
			if(animate){
				FWDAnimation.to(self.mainContentHolder_do, .8, {y:self.stageHeight, ease:Expo.easeInOut});
		...
		self.hideWithDelay = function(){
			...
			FWDAnimation.to(self.background_sdo, .6, {alpha:0});
		...
		self.destroy = function(){
			...
			if(self.scrollBar_do){
				FWDAnimation.killTweensOf(self.scrollBar_do);
				FWDAnimation.killTweensOf(self.scrollBarHandler_sdo);
				FWDAnimation.killTweensOf(self.scrollBarTrack_sdo);
			...
			FWDAnimation.killTweensOf(self.contentHolder_sdo);
...
(function (){
	...
	var FWDImageManager = function(data, parent){
		...
		this.disableMouseWheelZoom_bl = data.disableMouseWheelZoom_bl;
		...
		this.stopRotationHorizontalAtEnds_bl = data.stopRotationHorizontalAtEnds_bl;
		this.stopRotationVerticalAtEnds_bl = data.stopRotationVerticalAtEnds_bl;
		this.isMobile_bl = data.isMobile_bl;
		...
		this.dragVertical_bl = data.dragVertical_bl;
		this.isSwiping_bl = false;
		this.showLargeImageVersionOnZoom_bl = data.showLargeImageVersionOnZoom_bl;
		this.hasPointerEvent_bl = FWDUtils.hasPointerEvent;
		
		self.init = function(){
			if(self.controllerVerticalPosition_str == FWDController.POSITION_TOP && !self.useEntireScreenFor3dObject_bl) self.setY(self.controllerHeight);
		...
		self.resizeAndPositionCorrectionLines = function(animate){
			...
			FWDAnimation.killTweensOf(self.left_sdo);
			FWDAnimation.killTweensOf(self.top_sdo);
			FWDAnimation.killTweensOf(self.right_sdo);
			FWDAnimation.killTweensOf(self.bottom_sdo);
		
			if(animate){
				FWDAnimation.to(self.left_sdo, .2, {x:self.finalX, y:self.finalY, h:self.finalHeight});
				FWDAnimation.to(self.top_sdo, .2, {x:self.finalX, y:self.finalY, w:self.finalWidth});
				FWDAnimation.to(self.right_sdo, .2, {x:self.finalX + self.finalWidth - 2, y:self.finalY, h:self.finalHeight});
				FWDAnimation.to(self.bottom_sdo, .2, {x:self.finalX, y:self.finalY  + self.finalHeight - 2, w:self.finalWidth});
			}else{
		...
		this.initImages = function(){
			self.playListData_ar = data.playListData_ar;
			self.totalSets = self.playListData_ar.length;
			self.totalImages =  self.playListData_ar[0].length;
			self.curSet = data.startAtSet;
			self.image_ar = [];
			
			for(var i=0; i<self.totalSets; i++){
				self.image_ar[i] = [];
			}
			if(self.playListData_ar > 1) self.dragVertical_bl = false;
		}
		...
		self.activate = function(){
			...
			self.resizeAndPositionAfterAllLoad();
			self.addPannSupport();
			self.addPinchAndZoom();
			self.addRotationSupport();
			if(!self.disableMouseWheelZoom_bl) self.addMouseWheelSupport();
		...
		self.addSmallImage = function(){
			if(self.curId == self.prevId && self.curSet == self.prevSet) return;
			...
			if(self.addImageFirstTimeOnActivate_bl){
				self.removeSmallSDOId_to = setTimeout(function(){
						...
						self.smallImage_sdo = self.smallDos_ar[(self.curSet * self.totalImages) + self.curId];
			...
			if(self.largeImage_img){
				...
				if(!FWDUtils.isIEAndLessThen9) self.largeImage_img.src = "";
			}
		...
		self.addLargeImage = function(){	
			if(!self.playListData_ar[self.curSet][self.curId]["largeImagePath"]) return;
			if(!self.showLargeImageVersionOnZoom_bl) return;
			if(self.currentScale <= 1){
				self.largeImage_sdo.setVisible(false);
				return;
			}else{
				self.largeImage_sdo.setVisible(true);
			}
			...
			self.largeImage_img.src = self.playListData_ar[self.curSet][self.curId]["largeImagePath"];
		...
		self.gotoImage = function(){
			if(self.stopRotationHorizontalAtEnds_bl){
				if(self.curId < 0){
					self.curId = 0;
			...
			if(self.stopRotationVerticalAtEnds_bl){
				if(self.curSet < 0){
					self.curSet = 0;
				}else if(self.curSet > self.playListData_ar.length - 1){
					self.curSet = self.playListData_ar.length - 1
				}
			}else{
				if(self.curSet > self.playListData_ar.length - 1){
					self.curSet = 0;
				}else if(self.curSet < 0){
					self.curSet = self.playListData_ar.length - 1;
				}
			}
			...
			self.dispatchEvent(FWDImageManager.ROTATE_UPDATE, {image:self.images_ar[(self.curSet * self.totalImages) + self.curId]});
		};
		...
		self.resizeAndPositionSmallImage = function(animate){
			...
			FWDAnimation.killTweensOf(self.mainImagesHolder_do);
			FWDAnimation.killTweensOf(self.smallImage_sdo);
			clearTimeout(self.tweenDone_to);
			if(animate){
				FWDAnimation.to(self.mainImagesHolder_do, .2, {x:self.finalX, y:self.finalY});
				FWDAnimation.to(self.smallImage_sdo, .2, {w:self.finalWidth, h:self.finalHeight});
		...
		self.resizeAndPositionLargeImage = function(animate){
			...
			FWDAnimation.killTweensOf(self.largeImage_sdo);
			if(animate){
				FWDAnimation.to(self.largeImage_sdo, .2, {w:self.finalWidth, h:self.finalHeight});
			}else{
				self.largeImage_sdo.setWidth(self.finalWidth);
				self.largeImage_sdo.setHeight(self.finalHeight);
			}
		...
		//###############################################//
		this.addPinchAndZoom = function(){
			if(self.hasPointerEvent_bl){
				this.screen.addEventListener("MSPointerDown", this.onPinchTouchStart);
			}else{
				this.screen.addEventListener("touchstart", this.onPinchTouchStart);
			}
		};	
		
		this.removeZoomDragAndSwipeForMobile = function(){
			if(self.hasPointerEvent_bl){
				this.screen.removeEventListener("MSPointerDown", this.onPinchTouchStart);
		...
		this.onPinchTouchStart = function(e){
			...
			self.lastMouseX = self.firstTouchMouseX = e.touches[0].pageX - parent.main_do.getGlobalX() - parent.scrollOffsets.x;
			self.lastMouseY = self.firstTouchMouseY = e.touches[0].pageY - parent.main_do.getGlobalY() - parent.scrollOffsets.y;
			
			if(e.touches && e.touches.length == 2){
				self.disableOnDragOrSwipe_bl = true;
				self.lastMouseX2 = e.touches[1].pageX - parent.main_do.getGlobalX() - parent.scrollOffsets.x;
				self.lastMouseY2 = e.touches[1].pageY - parent.main_do.getGlobalY() - parent.scrollOffsets.y;
		...
		self.rotateStartHandler = function(e){
			...
			if(!self.isMobile_bl || e.pointerType == 'mouse') parent.showRotateDumy();
			
			self.currentDistX = self.lastPresedX = viewportMouseCoordinates.screenX;
			self.currentDistY = self.lastPresedY = viewportMouseCoordinates.screenY;
			self.showOrHideMarkers();
			self.positionMarkers();
		...
		self.rotateMoveHandler = function(e){
			...
			var viewportMouseCoordinates = FWDUtils.getViewportMouseCoordinates(e);
			var difX = (viewportMouseCoordinates.screenX - self.lastPresedX)/(80 * self.rotationSpeed); 
			var difY = (viewportMouseCoordinates.screenY - self.lastPresedY)/(80 * self.rotationSpeed); 
			
			
			self.swipeDragDistX = viewportMouseCoordinates.screenX - self.currentDistX;
			self.currentDistX = viewportMouseCoordinates.screenX;
			self.swipeDragDistY = viewportMouseCoordinates.screenY - self.currentDistY;
			self.currentDistY = viewportMouseCoordinates.screenY;
			
			if(self.dragVertical_bl){
				...
				if(Math.abs(difY) >= 1){
					if(difY > 0){
						self.lastPresedY = self.lastPresedY + (80 * self.rotationSpeed);
						if(difY != 0) difY = -1;
					}else{
						self.lastPresedY = self.lastPresedY - (80 * self.rotationSpeed);
						if(difY != 0) difY = 1;
		...
		self.rotateEndHandler = function(e){
			if(self.dragVertical_bl){
				if(Math.abs(self.swipeDragDistY) > self.spinDist && self.addDragAndSpinSupport_bl && self.firstInteractionOccured_bl){
					self.startToSwipe();
				}else{
					self.addLargeImage();
				}
			}else{
				if(Math.abs(self.swipeDragDistX) > self.spinDist && self.addDragAndSpinSupport_bl && self.firstInteractionOccured_bl){
					self.startToSwipe();
				}else{
					self.addLargeImage();
				}
		...
		this.startToSwipe = function(){
			...
			if(self.dragVertical_bl){
				self.swipeDirection_str = self.swipeDragDistY < 0 ? "top" : "bottom";
				self.swipeDragDistY = Math.abs(self.swipeDragDistY/(20 * self.dragAndSpinSpeed));
			}else{
				self.swipeDirection_str = self.swipeDragDistX < 0 ? "left" : "right";
				self.swipeDragDistX = Math.abs(self.swipeDragDistX/(20 * self.dragAndSpinSpeed));
			}
		...
		this.swipeTweenUpdate = function(){
			...
			if(self.dragVertical_bl){
				self.swipeDragDistY += (0 - self.swipeDragDistY) * 0.07;
				steps = Math.round(self.swipeDragDistY);
				if(self.swipeDirection_str == "top"){
					sgn = 1;
				}else{
					sgn = -1;
				}
				...
				self.swipeDragDistX += (0 - self.swipeDragDistX) * 0.07;
				steps = Math.round(self.swipeDragDistX);
		...
		self.markerOnMouseOutHandler = function(e){
			...
			if(marker.hasToolTip_bl){
				if(self.markersToolTip_do){
					...
					try{
						document.removeChild(self.markersToolTip_do.screen);
					}catch(e){}
		...
		self.showOrHideMarkers = function(){
			var marker;
			var positionObj = self.playListData_ar[self.curSet][self.curId].markersPosition_ar;
			var positonObj2;
			var totalCurMarkers;
		...
		self.setupMarkersToolTip = function(){
			...
			self.markersToolTip_do = new FWDMarkerToolTip(
					...
					self.buttonToolTipTopPointer_str,
					self.buttonToolTipBottomPointer_str);
			self.markersToolTip_do.getStyle().zIndex = 99999999;
		};
		...
		self.setupMarkersToolTipWindow = function(){
			...
			self.markersToolTipWindow_do = new FWDMarkerWindowToolTip(
					...
					self.buttonToolTipTopPointer_str,
					self.buttonToolTipBottomPointer_str);
			self.markersToolTipWindow_do.screen.style.zIndex = 99999999;
			//self.addChild(self.markersToolTipWindow_do);
			//console.log(self.markersToolTipWindow_do.screen);
			document.documentElement.appendChild(self.markersToolTipWindow_do.screen);
		};
		...
		self.showMarkerToolTip = function(marker, label){
			...
			document.documentElement.appendChild(self.markersToolTip_do.screen);
			...
			self.showMarkerToolTipId_to = setTimeout(function(){
				...
				finalY = marker.finalY - self.markersToolTip_do.totalHeight  - self.markerToolTipOffsetY + self.getGlobalY() + parent.scrollOffsets.y;
				
				if(finalY - parent.scrollOffsets.y < 0){
					finalY = marker.finalY + marker.height + self.markerToolTipOffsetY + self.getGlobalY() + parent.scrollOffsets.y + self.markersToolTip_do.pointerHeight;
				...
				finalX = parseInt(marker.finalX + (marker.width - self.markersToolTip_do.totalWidth)/2) + self.getGlobalX() + parent.scrollOffsets.x;
			
				if(finalX < 0){
					pointerOffsetX = finalX;
					finalX = 0;
				}else if(finalX + self.markersToolTip_do.totalWidth > parent.ws.w){
					pointerOffsetX = -(parent.ws.w - finalX - self.markersToolTip_do.totalWidth);
					finalX = parent.ws.w - self.markersToolTip_do.totalWidth;	
				}
		...
		self.showToolTipWindow = function(marker, label, maxWidth){
			...
			self.showToolTipWindoId_to = setTimeout(function(){
				...
				finalY = marker.finalY - self.markersToolTipWindow_do.totalHeight  - self.markerToolTipOffsetY + self.getGlobalY() + parent.scrollOffsets.y;
				
			
				if(finalY - parent.scrollOffsets.y < 0){
					finalY = marker.finalY + marker.height + self.markerToolTipOffsetY + self.getGlobalY() + parent.scrollOffsets.y + self.markersToolTipWindow_do.pointerHeight;
				...
				finalX = parseInt(marker.finalX + (marker.width - self.markersToolTipWindow_do.totalWidth)/2) + self.getGlobalX() + parent.scrollOffsets.x;
			
				if(finalX < 0){
					pointerOffsetX = finalX;
					finalX = 0;
				}else if(finalX + self.markersToolTipWindow_do.totalWidth > parent.ws.w){
					pointerOffsetX = -(parent.ws.w - finalX - self.markersToolTipWindow_do.totalWidth);
					finalX = parent.ws.w - self.markersToolTipWindow_do.totalWidth;	
				}
...
(function (window){
	...
	var FWDInfo = function(parent, warningIconPath){
		...
		this.init = function(){
			...
			self.textHolder_do = new FWDDisplayObject("div");
			if(!FWDUtils.isIEAndLessThen9) self.textHolder_do.getStyle().font = "Arial";
			...
			this.img_do = new FWDDisplayObject("img");
...
(function (window){
	...
	var FWDLightBox = function(
		...
		this.setupCloseButton = function(n_img, s_img){
			...
			FWDAnimation.to(self.closeButton_do, .9, {x:closeButtonFinalX, onComplete:self.showCloseButtonComplete, ease:Expo.easeInOut});
		...
		this.closeButtonOnStartHandler = function(e){
			...
			FWDAnimation.to(self.closeButton_do, .9, {x:viewportSize.w, ease:Expo.easeInOut});
		...
		self.destroy = function(){
			...
			if(self.lightBoxBackground_sdo){
				FWDAnimation.killTweensOf(self.lightBoxBackground_sdo);
				self.lightBoxBackground_sdo.destroy();
			}
			
			if(self.lightBoxGridHolder_do){
				FWDAnimation.killTweensOf(self.lightBoxGridHolder_do);
...
var FWDMarker = function(
		...
		self.onMouseOver = function(e){
			if(!e.pointerType || e.pointerType == e.MSPOINTER_TYPE_MOUSE){
				...
				FWDAnimation.to(self.s_do, .5, {alpha:1, delay:.1, ease:Expo.easeOut});
			}
		};
		...
		self.show = function(){
			...
			self.showId_to = setTimeout(function(){
				...
				//if(!FWDUtils.isMobile){
					FWDAnimation.to(self.n_do, .8, {alpha:1, ease:Quart.easeOut});
				//}
		...
		self.showOnChromeOnce = function(){
			self.n_do.setAlpha(0);
			FWDAnimation.to(self.n_do, .8, {alpha:1, delay:.1, ease:Quart.easeOut});
		};
...
var FWDMarkerWindowToolTip = function(
		...
		self.hide = function(){
			if(!self.isShowed_bl) return;
			if(!self.isMobile_bl) FWDAnimation.killTweensOf(self);
			self.setX(-5000);
			self.setAlpha(0);
...
var FWDNavigator = function(parent, data){
		...
		this.totalHeight = Math.round(self.totalWidth * (data.imageHeight/data.imageWidth));
		...
		self.activate = function(){
			...
			self.updateImage(parent.imageManager_do.images_ar[parent.imageManager_do.curId]);
			self.addPannSupport();
		...
		self.setupImagesSdos = function(start){
			self.image_canvas = new FWDSimpleDisplayObject("canvas");
			self.image_canvas.screen.width = self.totalWidth;
			self.image_canvas.screen.height = self.totalHeight;
			
			//self.image_canvas.getStyle().backgroundColor = "#FFFF00";
			self.canvas_ctx = self.image_canvas.screen.getContext("2d");
			self.mainHolder_do.addChildAt(self.image_canvas, 0);
		};
		...
		self.updateImage = function(image){
			...
			self.canvas_ctx.clearRect(0, 0, self.image_canvas.width, self.image_canvas.height);
			self.canvas_ctx.drawImage(image,0,0,data.imageWidth,data.imageHeight,0,0,self.totalWidth,self.totalHeight);
		};
		...
		self.update = function(percentX, percentY, percentWidth, percentHeight, animate){
			...
			if(animate){
				...
				FWDAnimation.killTweensOf(self.handler_sdo);
				FWDAnimation.to(self.handler_sdo, .2, {x:self.finalX, y:self.finalY, w:self.finalWidth, h:self.finalHeight});
			}else{
				self.isTweening_bl = false;
				FWDAnimation.killTweensOf(self.handler_sdo);
		...
		self.hide = function(animate){
			...
			if(self.navigatorPosition_str == FWDNavigator.TOP_LEFT || self.navigatorPosition_str == FWDNavigator.TOP_RIGHT){
				if(animate){
					FWDAnimation.to(self.mainHolder_do, 1, {y:-self.totalHeight - self.offsetY, ease:Expo.easeInOut});
...
(function (window){
	...
	var FWDPreloader = function(
		...
		self.show = function(){
			...
			FWDAnimation.to(self.bk_do, 1, {alpha:.6});
			FWDAnimation.to(self.animHolder_do, .8, {y:0, ease:Expo.easeInOut});
			self.isShowed_bl = true;
		};
		...
		self.hide = function(animate){
			if(self == null) return;
			FWDAnimation.killTweensOf(self);
			FWDAnimation.killTweensOf(self.animHolder_do);
			if(animate){
				self.allowResize_bl = false;
				FWDAnimation.to(self.bk_do, .8, {alpha:0, delay:.6, onComplete:self.onHideComplete});
				FWDAnimation.to(self.animHolder_do, .8, {y:self.stageHeight/2 +  self.segmentHeight, ease:Expo.easeInOut});
			}else{
				self.bk_do.setAlpha(0);
...
var FWDSimpleButton = function(nImg, sImg){
		...
		self.setSelctedFinal = function(){
			...
			FWDAnimation.killTweensOf(self.s_do);
			FWDAnimation.to(self.s_do, .8, {alpha:1, ease:Expo.easeOut});
			self.setButtonMode(false);
		};
		
		self.setUnselctedFinal = function(){
			self.isSelectedFinal_bl = false;
			FWDAnimation.to(self.s_do, .8, {alpha:0, delay:.1, ease:Expo.easeOut});
			self.setButtonMode(true);
		};
...
(function (window){
	...
	var FWDViewer = function(props){
		...
		self.init = function(){
			...
			this.warningIconPath_str = this.skinPath_str + "skin/warning.png";
			...
			self.autoScale = self.props_obj.autoScale == "yes" ? true : false;
			self.originalWidth = self.props_obj.viewerWidth;
			self.originalHeight = self.props_obj.viewerHeight;
			...
			if(self.displayType != FWDViewer.LIGHTBOX){
				...
				if(self.initializeOnlyWhenVisible_bl){
					...
					self.scrollInit_to = setTimeout(self.onInitlalizeScrollHandler, 500);
				}else{
					self.setupViewer();
				}
			}else{
		...
		self.onInitlalizeScrollHandler = function(){
			...
			if(self.main_do.getGlobalY() >= -self.stageHeight && self.main_do.getGlobalY() < self.ws.h){
		...
		self.setupViewer = function(){
			...
			clearTimeout(self.scrollInit_to);
			self.setupInfo();
			self.setupData();
		...
		self.setupMainDo = function(){
			...
			self.main_do = new FWDDisplayObject("div", "absolute");
		...
		self.setupInfo = function(){
			...
			self.info_do = new FWDInfo(self, self.warningIconPath_str);
		};	
		...
		self.resizeHandler = function(overwrite){
			...
			if(self.displayType == FWDViewer.LIGHTBOX && !self.isFullScreen_bl){
				...
				if(window.innerWidth >= 1002){
					self.main_do.setX(scrollOffsets.x+300);
					self.main_do.setY(scrollOffsets.y);
					self.stageWidth = viewportSize.w - 300;
				...
				if (self.autoScale){
					...
					self.stageHeight = Math.max(parseInt(self.scale * self.originalHeight), 200);
					...
					self.stageWidth = parseInt(self.stageContainer.offsetWidth);
		...
		self.setupContextMenu = function(){
			if(self.data.buttons_ar.length == 0) return;
		...
		self.firstImageLoadComplete = function(){
			...
			FWDAnimation.to(self.imageManager_do, .6, {alpha:1});
		...
		self.dataSkinProgressHandler = function(e){
			if(self.preloader_do) self.preloader_do.update(e.percent, "Loading skin:" + (Math.round(e.percent * 100)) + "%");
		};
		...
		self.dataImagesLoadComplete = function(){
			...
			if(self.dispatchEvent) self.dispatchEvent(FWDViewer.All_IMAGES_LOADED);
		...
		self.setupController = function(){
			...
			self.controller_do.addListener(FWDController.GOTO_TOP_OR_BOTTOM_IMAGE, self.gotoTopOrBottomImageHandler);
		...
		self.gotoTopOrBottomImageHandler = function(e){
			if(!self.data.areAllImagesLoaded_bl) return;
			self.imageManager_do.curSet += e.dir;
		...
		self.imageManagerRotateUpdateHandler = function(e){
			if(self.navigator_do) self.navigator_do.updateImage(e.image);
		...
		self.imageManagerUpdateNavigatorHandler = function(e){
			self.percentX = e.percentX;
			self.percentY = e.percentY;
			self.percentWidth = e.percentWidth;
			self.percentHeight = e.percentHeight;
		...
		this.setupRotationDumy = function(){
			...
			if(self.data.dragInAllDirection_bl){
				self.rotateDumy_sdo.getStyle().cursor = 'url(' + self.data.handGrabPath_str + '), default';
			}else if(self.data.dragVertical_bl){
				self.rotateDumy_sdo.getStyle().cursor = 'url(' + self.data.handGrabRotateVerticalPath_str + '), default';
			}else{
		...
		self.goFullScreen = function(){
			...
			self.main_do.getStyle().zIndex = 9999999;
		...
		self.addMainDoToTheOriginalParent = function(){
			...
			if(self.navigator_do){
				setTimeout(function(){
					self.navigator_do.update(0.5, 0.5, self.percentWidth,self.percentHeight,false);
				}, 200);
			}
		...
		this.playToImage = function(id, callBack){
			if(self.safeToControll_bl) self.controller_do.playToImage(id, callBack);
		};
		
		
		this.hideInfoWindow = function(){
			self.descriptionWindow_do.hide(true);
		}
		...
		self.destroy = function(){
			...
			if(self.imageManager_do){
				FWDAnimation.killTweensOf(self.imageManager_do);
